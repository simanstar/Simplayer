...
void ScreenShoot::setPlayStateChanged(bool changed)
{
    playStateChanged=changed;
}
void ScreenShoot::setSimPlayer(SimPlayer *simPlayer)
{
...
//    QString picName = str + ".bmp";
    ...
    // 对全屏背景进行截图
    QString fileName = QFileDialog::getSaveFileName(this,tr("save Image"),BaseSetting::getSnapShotUrl(),tr("Picture(*.png)"));//""处可以设置默认路径
    if(!fileName.isEmpty())
    {
        QImage pic = bg->copy(x,y,wid,height);
        if(!pic.save(fileName))
        {
            QMessageBox::information(this,tr("Failed to save the iamge"),tr("Failed to save the iamge!"));
            QFile::remove("bg.bmp");
            return;
        }
        ...
        BaseSetting::setSnapShotUrl(file_path);
    //    QFile::remove("bg.bmp");
        // 获取程序当前路径，打片截图
